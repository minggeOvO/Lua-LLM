...
def prune_pruner_zero_sp(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0, engine=None):
    use_cache = model.config.use_cache 
    model.config.use_cache = False 
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            # # sum of the metric for each column
            # W_column = W_metric.sum(dim=0)
            # # standardization
            # W_column = (W_column - W_column.mean()) / W_column.std()
            # dict_key = f"{i}_{name}"
            # col_saliency_dict[dict_key] = W_column

            # # sum of the metric for each row
            # W_row = W_metric.sum(dim=1)
            # # standardization
            # W_row = (W_row - W_row.mean()) / W_row.std()
            # dict_key = f"{i}_{name}"
            # row_saliency_dict[dict_key] = W_row

            W_mask = (torch.zeros_like(W_metric) == 1)  ## initialize a mask to be all False
            
            # unstructured pruning column
            sort_res = torch.sort(W_metric, dim=-1, stable=True)
            indices = sort_res[1][:,:int(W_metric.shape[1]*args.sparsity_ratio)]
            W_mask.scatter_(1, indices, True)

            if i == 0:
                # plot W_mask to see the sparsity pattern
                import numpy as np
                W_np = W_mask.cpu().numpy()
                import matplotlib.pyplot as plt
                plt.imshow(W_np, cmap="binary_r", interpolation="nearest")
                plt.title(f"{name}")

                plt.xticks([])  # 取消x轴的刻度
                plt.yticks([])  # 取消y轴的刻度
                plt.savefig(f"llama1_column_sparsity_pattern_layer_{i}_{name}.svg", dpi= 300,format='svg')
                
                

            #subset[name].weight.data[W_mask] = 0  ## set weights to zero 
            col_sparsity = W_mask.sum(dim=0).float() / W_mask.shape[0]
            print(col_sparsity)
            # col_mask = col_sparsity > 0.9
            # W_mask[:,col_mask] = 0

            dict_key = f"{i}_{name}"
            col_sparsity_dict[dict_key] = col_sparsity

            # # unstructured pruning row
            ...
            # #     import matplotlib.pyplot as plt
            # #     plt.imshow(W_np, cmap="binary_r", interpolation="nearest")
            # #     plt.savefig(f"row_sparsity_pattern_layer_{i}_{name}.png")
            # row_sparsity = W_mask.sum(dim=1) / W_mask.shape[1]
            # dict_key = f"{i}_{name}"
            # row_sparsity_dict[dict_key] = row_sparsity
    ...
    # save col_sparsity_dict
    torch.save(col_sparsity_dict, '1-col_sparsity_dict.pt')
    #torch.save(row_sparsity_dict, 'row_sparsity_dict.pt')
    # torch.save(col_saliency_dict, 'col_saliency_dict.pt')
    # torch.save(row_saliency_dict, 'row_saliency_dict.pt')
